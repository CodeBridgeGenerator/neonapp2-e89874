...
const CustomerDetailsFakerDialogComponent = (props) => {
    ...
    useEffect(() => {
        ...
        client.service("customerAddress")
                        .find({ query: { $select: ["_id","addressType","addressLine1","addressLine2","city","postalCode"] } })
                        .then((data) => {
    ...
    const onRun = async () => {
        let fakeData = customerDetailsFakerFactory(props.user,fakerCount,customerAddressIds);
